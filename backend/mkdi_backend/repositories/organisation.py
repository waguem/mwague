...
class OrganizationRepository:
    ...
    async def create_organization(self,input):
        org:Organization = self.db.query(Organization).filter(Organization.initials == input.initials).first()
        if org:
            raise MkdiError(
                f"Organization {input.initials} already exists", error_code=MkdiErrorCode.ORGANIZATION_EXISTS
            )
        org = Organization(
