...
export default function Wallets({ office }: Props) {
    ...
    const handleCreateWalled:MRT_TableOptions<OfficeWalletResponse>["onCreatingRowSave"] = async ({exitCreatingMode})=>{
    ...
    const columns = useMemo<MRT_ColumnDef<OfficeWalletResponse>[]>(() => [
        {
            ...
            enableEditing:false,
            Cell: ({ row}) => (
                <Badge variant="gradient" size="lg" gradient={{
                    from:"teal",
                    to:"cyan",
                    deg:90
                }}>
        ...
        },
        {
            ...
            enableEditing:false,
            Cell: ({cell,row}) => {
                return <NumberFormatter prefix={getMoneyPrefix(row.original.wallet_currency)} value={cell.getValue() as number} decimalScale={4} thousandSeparator=","/>
            }
        },
        {
            header:"Paid",
            ...
            enableEditing:false,
            Cell: ({cell,row}) => {
                return <NumberFormatter prefix={getMoneyPrefix(row.original.payment_currency)} value={cell.getValue() as number} decimalScale={4} thousandSeparator=","/>
            }
        },
        {
            header:"Buying Rate",
